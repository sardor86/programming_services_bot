...
async def get_github_project_links(message: Message, state: FSMContext) -> None:
    ...
    phone_number_client = ProgrammerWork(message.bot.get('config').db).get_work(message.from_user.id).client_phone_number

    logger.info('get client id')
    client_id = Users(message.bot.get('config').db).get_all_information_user_phone_number(phone_number_client).user_id
